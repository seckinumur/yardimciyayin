    ...
    public class SiparisRepo
    {
        ...
        public static void SiparisSil(int ID)
        {
            using (DBCON db = new DBCON())
            {
                ...
                db.UrunSepet.RemoveRange(Sepet.UrunSepet);
        ...
        public static bool SiparisGonder(int ID)
        {
            using (DBCON db = new DBCON())
            {
                try
                {
                    ...
                    Siparis.Yil = DateTime.Now.Year.ToString();
                    Siparis.Ay = DateTime.Now.Month.ToString();
                    Siparis.Gun = DateTime.Now.Day.ToString();
        ...
        public static List<VMGonderilen> GonderilenSiparislerListe() //Beta
        {
            using (DBCON db = new DBCON())
            {
                return db.Siparis.Where(p => p.Gonderildimi == true && p.İptal != true).OrderBy(n => n.Yil).OrderBy(g => g.Ay).OrderBy(u => u.Gun).Select(p => new VMGonderilen
                {
                    Yil = p.Yil,
                    Ay = p.Ay,
                    Gun = p.Gun
